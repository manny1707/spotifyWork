let exerciseVideos = { 
    ...
    "98": ["6FEDrU85FLE", "b1c6i0VT7ak", "o9Rl3zoi554", "laE2mvwZN8w", "Z4o7-6cFUF8"], 
...
var exerciseNamesOnPage = document.getElementsByClassName('exercise-name');
var exerciseDescriptionsOnPage = document.getElementsByClassName('exercise-desc');
var exerciseVideosOnPage = document.getElementsByClassName('youtubeVideo');
...
var addBtn1 = document.getElementById('addToPlanBtn1');
var addBtn2 = document.getElementById('addToPlanBtn2');
var addBtn3 = document.getElementById('addToPlanBtn3');

var allAddBtns = document.getElementsByClassName('t-button-2');
...
let queryExercises = async () => {
    ...
    await fetch(`https://wger.de/api/v2/exercise/?muscles=1&license_author=wger.de&language=2`, apiRequest)
        .then(apiResponse => apiResponse.text())
        .then(apiResult => {
            exerciseResults = JSON.parse(apiResult);
            exerciseArr = exerciseResults.results;
            ...
            for (let i = 0; i < exerciseNamesOnPage.length; i++) {
                exerciseNamesOnPage[i].innerHTML = exerciseNames[i];
                exerciseDescriptionsOnPage[i].innerHTML = exerciseDescriptions[i];
                let uniqueVideoIds = exerciseVideos[exerciseIds[i]];
                exerciseVideosOnPage[i].src = 'https://www.youtube.com/embed/' + uniqueVideoIds[0];

                let associatedBtn = allAddBtns[i];
                if (userWorkoutPlan.exercises.includes(exerciseIds[i])) {
                    associatedBtn.style.backgroundColor = 'blue';
                    associatedBtn.innerHTML="Remove from Workout";
                    associatedBtn.setAttribute('onclick', `removeExerciseFromPlan(${i+1})`);
                }
            }
        })
...
let addExerciseToPlan = async (btnNum) => {
    let exerciseIdToAdd = null;
    
    if (btnNum == 1) {
        exerciseIdToAdd = exerciseIds[currentStartIndex];
        addBtn1.style.backgroundColor = 'blue';
        addBtn1.innerHTML="Remove from Workout";
        addBtn1.setAttribute('onclick', 'removeExerciseFromPlan(1)');
    }
    else if (btnNum == 2) {
        exerciseIdToAdd = exerciseIds[currentStartIndex + 1];
        addBtn2.style.backgroundColor = 'blue';
        addBtn2.innerHTML="Remove from Workout";
        addBtn2.setAttribute('onclick', 'removeExerciseFromPlan(2)');
    }
    else if (btnNum == 3) {
        exerciseIdToAdd = exerciseIds[currentStartIndex + 2];
        addBtn3.style.backgroundColor = 'blue';
        addBtn3.innerHTML="Remove from Workout";
        addBtn3.setAttribute('onclick', 'removeExerciseFromPlan(3)');
    }

    userWorkoutPlan.exercises.push(exerciseIdToAdd);
    await updateWorkoutPlan();
}

let removeExerciseFromPlan = async (btnNum) => {
    let exerciseIdToAdd = null;
    
    if (btnNum == 1) {
        exerciseIdToRemove = exerciseIds[currentStartIndex];
        addBtn1.style.backgroundColor = 'red';
        addBtn1.innerHTML="Add to Workout";
        addBtn1.setAttribute('onclick', 'addExerciseToPlan(1)');
    }
    else if (btnNum == 2) {
        exerciseIdToRemove = exerciseIds[currentStartIndex + 1];
        addBtn2.style.backgroundColor = 'red';
        addBtn2.innerHTML="Add to Workout";
        addBtn2.setAttribute('onclick', 'addExerciseToPlan(2)');
    }
    else if (btnNum == 3) {
        exerciseIdToRemove = exerciseIds[currentStartIndex + 2];
        addBtn3.style.backgroundColor = 'red';
        addBtn3.innerHTML="Add to Workout";
        addBtn3.setAttribute('onclick', 'addExerciseToPlan(3)');
    }

    let indexToRemove =  userWorkoutPlan.exercises.indexOf(exerciseIdToRemove);
    userWorkoutPlan.exercises.splice(indexToRemove, 1);
    await updateWorkoutPlan();
}

let updateWorkoutPlan = async () => {
    let apiHeaders = new Headers();
    let apiBody = null;
    let cookies = document.cookie;
    let username = cookies.split('; ').find(element => element.startsWith('username')).split('=')[1];
    ...
    apiHeaders.append('Content-Type', 'application/json');
    apiBody = JSON.stringify({ "username": username, "updateType": 'workoutPlan', "newWorkoutPlan": JSON.stringify(userWorkoutPlan) });
...
span1.onclick = function() {
    ...
    uniqueVideoIds = exerciseVideos[exerciseIds[currentStartIndex]];
    exerciseVideosOnPage[0].src = 'https://www.youtube.com/embed/' + uniqueVideoIds[0];
}

span2.onclick = function() {
    tModal2.style.display = "none";
    uniqueVideoIds = exerciseVideos[exerciseIds[currentStartIndex+1]];
    exerciseVideosOnPage[1].src = 'https://www.youtube.com/embed/' + uniqueVideoIds[0];
}

span3.onclick = function() {
    tModal3.style.display = "none";
    uniqueVideoIds = exerciseVideos[exerciseIds[currentStartIndex+2]];
    exerciseVideosOnPage[2].src = 'https://www.youtube.com/embed/' + uniqueVideoIds[0];
}
